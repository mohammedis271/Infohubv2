...
public class RecylerViewAdapterCastsPage extends RecyclerView.Adapter<RecylerViewAdapterCastsPage.ViewHolder> {

    Context context;
    SelectedMovieCallHandler sm;
    String query,cast_crew;

    public RecylerViewAdapterCastsPage(Context context,String query,String cast_crew,String section) {
    ...
    public void onBindViewHolder(@NonNull final ViewHolder holder, int position) {
        if (cast_crew.equals("cast")){
            String call = buildImageCall(1,"profile",3,sm.getCastProfilePaths().get(position));
            ...
            String call = buildImageCall(1,"profile",3,sm.getCrewProfilePaths().get(position));
            ImageLoader.getInstance()
                       .displayImage(call, holder.posterImage, new ImageLoadingListener() {
                           @Override
                           public void onLoadingStarted(String imageUri, View view) {
            ...
            holder.characterText.setText(sm.getCrewCharacters().get(position));
            holder.nameText.setText(sm.getCrewNames().get(position));
        }
    ...
    public int getItemCount() {
        if (cast_crew.equals("cast")){
            return sm.getCastCharacters().size();
        }else {
            return sm.getCrewCharacters().size();
        }
    }
